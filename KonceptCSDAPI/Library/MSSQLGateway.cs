...
namespace KonceptSupportLibrary
{
    public class MSSQLGateway
    {
        ...
        public MSSQLGateway(string conn_string)
        {
        ...
        //Function for  Return dataset with procedure name and parameters With Aysnc Method
        public async Task<DataTable> ExecuteProcedureAsyncMethod(String storedprocname, List<SqlParameter> _params)
        {
            SqlConnection conn = GetConnection();
            DataSet _ds = new DataSet();
            SqlCommand cmd = new SqlCommand();
            ...
            return await Task.Run(() =>_ds.Tables["DATATABLE"]);
        }
    }
}
