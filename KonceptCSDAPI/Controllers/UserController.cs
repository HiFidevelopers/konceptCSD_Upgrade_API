...
namespace KonceptCSDAPI.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public ServiceResponseModel DeleteUser([FromBody] UserDeleteModel model)
        {
            #region DATA VALIDATION
            if (model == null)
            {
            ...
            DataTable _dtresp = _IUserManager.deleteUser(model);
            if (_objHelper.checkDBResponse(_dtresp))
            {
                if (Convert.ToString(_dtresp.Rows[0]["response"]) == "0")
                {
        ...
        public ServiceResponseModel DeleteUserGroup([FromBody] UserGroupDeleteModel model)
        {
            #region DATA VALIDATION
            if (model == null)
            {
            ...
            DataTable _dtresp = _IUserManager.deleteUserGroup(model);
            if (_objHelper.checkDBResponse(_dtresp))
            {
                if (Convert.ToString(_dtresp.Rows[0]["response"]) == "0")
                {
        ...
        [HttpPost]
        [Route("fetchuserslotsavailability")]
        #region Fetch User Slots Availability

        public ServiceResponseModel fetchUserSlotsAvailability([FromBody] UserSlotsAvailabilityFilterModel model)
        {
            #region DATA VALIDATION
            if (model == null)
            {
            ...
            DataTable _dtresp = _IUserManager.fetchUserSlotsAvailability(model);
            if (_objHelper.checkDBResponse(_dtresp))
            {
                if (Convert.ToString(_dtresp.Rows[0]["response"]) == "0")
                {
