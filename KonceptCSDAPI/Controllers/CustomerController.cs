...
namespace KonceptCSDAPI.Controllers
{
	...
	public class CustomerController : ControllerBase
	{
		...
		[HttpPost]
		[Route("deletecustomerchild")]
		#region Delete Customer Child

		public ServiceResponseModel deleteCustomerChild([FromBody] CustomerChildDeleteModel model)
		{
			#region DATA VALIDATION
			if (model == null)
			{
			...
			DataTable _dtresp = _ICustomerManager.DeleteCustomerChild(model);
			if (_objHelper.checkDBResponse(_dtresp))
			{
				if (Convert.ToString(_dtresp.Rows[0]["response"]) == "0")
				{
